...
import { ChoqueDeTransaccionError, esChoqueDeTransaccion, esEsperaVencida } from '../../../shared/prisma/choque-de-transaccion.js'
...
//
// Las lecturas van por la transacción del candado: son de la misma conexión y no piden otra al
// pool, que ya lo usan la creación y la siembra. En Read Committed ven lo que esas confirmaron.
export const asegurarLibroPropio = async (auth: AbreLibros, prisma: PrismaClient, userId: string): Promise<string | null> => {
  try {
    return await prisma.$transaction(
      async (tx) => {
        ...
        if ((await tx.bookMember.count({ where: { userId } })) > 0) return null
        // Quien se está dando de baja no necesita libro.
        if (!(await tx.authUser.findUnique({ where: { id: userId }, select: { id: true } }))) return null
        ...
        const nuevo = await tx.bookMember.findFirstOrThrow({ where: { userId }, select: { organizationId: true } })
        // Una sesión parada en el libro que perdió contesta 403 a todo: pasa al nuevo.
        await tx.authSession.updateMany({ where: { userId }, data: { activeOrganizationId: nuevo.organizationId } })
