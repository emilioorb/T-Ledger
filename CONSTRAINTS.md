...
|---|---|---|---|
| Cobertura de líneas (api) | 89.27 | no baja | `node scripts/cobertura-del-proyecto.mjs` tras la suite completa |
| Cobertura de líneas (web) | 34.16 | no baja | ídem |
...
| E3 | Aserciones quitadas (20, guardia del piso) | `api`: `admin/domain/admin.spec.ts`, `identity/infrastructure/{registro,auth.config.e2e}.spec.ts`; `web`: `features/datos/{dar-acceso,enlace-de-invitacion}.spec.*` | Reescritas o mudadas, no perdidas. Cambió la firma de `puedeRegistrarse` (del correo al token del enlace), el admin pasó de correo a id y el enlace lleva token y correo en el fragmento. Por archivo: admin 9→9, registro 8→22, auth.config 9→27, dar-acceso 2→4. `enlace-de-invitacion` baja 4→2 porque se borró `correoDeLaBusqueda`; la lectura del fragmento se prueba ahora en `identity/token-del-enlace.spec.ts` (0→12). El borde «vence en este instante» que se había perdido se restituyó. Aceptada por Emilio el 2026-09-23. Deja de aplicar cuando la rama entre a `main`. | Emilio | 2026-10-07 |
| E4 | Aserciones quitadas (2, guardia del piso) | `api`: `accounting/infrastructure/prisma-account.repository.spec.ts`, `libro/infrastructure/libro.controller.spec.ts` | Reescritas, no perdidas (tarea 4b de la 6b). `SeedChartUseCase` recibe la transacción en su constructor (10→10). El spec del enlace dejó un rastro falso por el real, que pasa por el filtro de libro: el doble tapaba que el rastro se escribía sin transacción (7→8). Aceptada por Emilio el 2026-09-24. Deja de aplicar cuando la rama entre a `main`. | Emilio | 2026-10-08 |
| E5 | Aserciones quitadas (2, guardia del piso) | `api`: `libro/application/borrar-libro.use-case.spec.ts`, `libro/application/borrar-archivos-del-libro.listener.spec.ts` | Reescritas, no perdidas: el evento `LIBRO_BORRADO` ahora lleva quiénes eran los miembros (para abrirle un libro propio a quien se queda sin ninguno), y las dos aserciones que lo emitían o lo recibían cambiaron de forma. Aserciones: 7→7 y 2→2. Aceptada por Emilio el 2026-09-24. Deja de aplicar cuando la rama entre a `main`. | Emilio | 2026-10-08 |
