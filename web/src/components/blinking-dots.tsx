...
const DotField = ({
  ...
  useFrame((_, delta) => {
    ...
    // `material.uniforms` viene tipado como diccionario abierto, así que con
    // `noUncheckedIndexedAccess` cada acceso sería «posiblemente undefined». No lo es: el
    // material se construye con este mismo objeto (ver `uniforms={uniforms}` más abajo), así
    // que las claves existen por construcción. Decírselo a TypeScript acá evita tener que
    // repetir una guarda en cada una de las veinticuatro lecturas.
...
const BlinkingDots = ({
  ...
  useEffect(() => {
    ...
    const watcher = new IntersectionObserver(
      // `noUncheckedIndexedAccess` obliga a contemplar la lista vacía. Si el observador no
      // reporta nada, lo honesto es dar el elemento por no visible: dibujar un shader que
      // nadie está viendo es gastar batería.
      ([entry]) => setAwake(entry?.isIntersecting ?? false),
      { rootMargin: "120px" },
    );
